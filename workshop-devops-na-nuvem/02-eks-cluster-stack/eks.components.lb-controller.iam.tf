    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${local.eks_oidc_provider_url}:sub" = "system:serviceaccount:kube-system:aws-load-balancer-controller"
          }
          StringEquals = {
...
#Esse arquivo cria a policy para o load balancer controller conforme documentação
...
resource "aws_iam_policy" "load_balancer" {
  ...
  path        = "/"
  description = "IAM Policy para o AWS Load Balancer Controller"
